...
@pytest.fixture
def combat(player: Character, enemy: Enemy) -> Combat:
    """Create combat instance."""
    return Combat(player, enemy)


def test_combat_init(combat: Combat, player: Character, enemy: Enemy) -> None:
...
def test_roll_initiative_player_first(combat: Combat) -> None:
...
def test_roll_initiative_enemy_first(combat: Combat) -> None:
    """Test initiative when enemy wins."""
    with patch("dndgame.combat.roll", side_effect=[8, 12]):
        order = combat.roll_initiative()
    assert order[0] == combat.enemy


def test_attack_hit(combat: Combat, player: Character, enemy: Enemy) -> None:
    """Test successful attack."""
    initial_hp = enemy.hp
    with patch("dndgame.combat.roll", side_effect=[20, 5]):
        damage = combat.attack(player, enemy)
    assert damage > 0
    assert enemy.hp < initial_hp


def test_attack_miss(combat: Combat, player: Character, enemy: Enemy) -> None:
    """Test missed attack."""
    initial_hp = enemy.hp
...
def test_execute_round(combat: Combat) -> None:
    """Test round execution."""
    combat.initiative_order = [combat.player, combat.enemy]
    with patch("dndgame.combat.roll", side_effect=[15, 3, 15, 2]):
        should_continue = combat.execute_round()
    assert combat.round == 1


def test_combat_ends_when_defeated(combat: Combat, enemy: Enemy) -> None:
